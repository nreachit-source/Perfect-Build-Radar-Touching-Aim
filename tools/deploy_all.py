...
cp /var/mobile/Media/radar_overlay_next.dylib /var/jb/tmp/radar_overlay_staged.dylib
chmod 755 /var/jb/tmp/radar_overlay_staged.dylib
ldid -S /var/jb/tmp/radar_overlay_staged.dylib
OHASH=$(ldid -h /var/jb/tmp/radar_overlay_staged.dylib | grep -o 'CDHash=[0-9a-fA-F]*' | head -n1 | cut -d= -f2)
...
cp /var/jb/tmp/radar_overlay_staged.dylib /var/jb/usr/lib/TweakInject/radar_overlay.dylib

# Restart daemon
launchctl kickstart -k system/com.local.ue4loadmonitor || true
"""
    res = checked(sign_script)
    print("Daemon and tweak staged successfully.")

    print("==> 3. Restarting SpringBoard to cleanly attach overlay...")
    restart_script = """
killall -9 SpringBoard
sleep 3
"""
    try:
        run_script(restart_script, timeout=10)
    except Exception:
        pass

    time.sleep(4)
    status = run_script("ps -ef | grep -e SpringBoard -e ue4load | grep -v grep")
    print(status)
